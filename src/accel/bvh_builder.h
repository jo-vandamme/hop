...
public:
    ...
    static std::vector<Node> build(const std::vector<Object>& items, uint32 min_leaf_size, LeafCreationCallback callback);
...
template <typename Object, typename Accessor, typename ScoringStrategy>
std::vector<Node> Builder<Object, Accessor, ScoringStrategy>::build(
        ...
        const std::vector<Object>& items, int depth, BBoxr& node_bbox)
{
    ...
    node_bbox = BBoxr();
    for (auto& item : items)
        node_bbox.merge(Accessor::get_bbox(item));

    if (items.size() <= m_min_leaf_size)
        return create_leaf(&node, items);

    // Get current node score
    Real best_score = ScoringStrategy::score_partition(items);
...
#pragma omp parallel for
    ...
    for (auto& item : items)
    {
        Vec3r center = Accessor::get_centroid(item);
...
public:
    ...
    static Real score_split(const std::vector<Object>& items, uint8 axis, Real split_point, uint32* left_count, uint32* right_count)
    {
        BBoxr left_bbox, right_bbox;
    ...
    static Real score_partition(const std::vector<Object>& items)
    {
        if (items.empty())
            return pos_inf;
